# facilities/controllers/facility_controller.py
...
from facilities.schemas.facility_serializer import (
    FacilitySerializer,
    CreateFacilitySerializer,
    UpdateFacilitySerializer,
)
from utils.pagination import DefaultPagination
from utils.response_handler import success_response, error_response


# -------- Normalizador local de errores (mismo contrato que en Users) --------
def _as_list(val):
    if val is None:
        return []
...
class FacilityListCreateView(APIView):
    """
    Listado paginado y creación de Facilities.
    ...
    def get(self, request) -> Response:
        try:
            ...
            data = FacilitySerializer(page, many=True).data
    ...
    def post(self, request) -> Response:
        try:
            ...
            data = FacilitySerializer(inst).data
...
class FacilityDetailView(APIView):
    """
    Detalle / actualización / borrado lógico de un Facility.
    - GET /facilities/<id>/
    - PUT/PATCH /facilities/<id>/
    - DELETE /facilities/<id>/  (soft-delete: is_deleted=True, is_active=False)
    """
    ...
    def get(self, request, pk: int) -> Response:
        try:
            ...
            return success_response(FacilitySerializer(inst).data, status.HTTP_200_OK)
        except DjangoValidationError:
            return error_response({"detail": ["Facility not found."]}, status.HTTP_404_NOT_FOUND)
        except Exception:
            return error_response({"detail": ["Internal server error"]}, status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def _update(self, request, pk: int, partial: bool) -> Response:
        ...
        serializer = UpdateFacilitySerializer(current, data=request.data, partial=partial)
        try:
            if not serializer.is_valid():
                return error_response(normalize_errors(serializer.errors), status.HTTP_400_BAD_REQUEST)

            updated = self.service.update(pk, serializer.validated_data)  # type: ignore
            return success_response(FacilitySerializer(updated).data, status.HTTP_200_OK)
